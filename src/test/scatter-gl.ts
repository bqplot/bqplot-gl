...
import * as bqplot_gl from '../index';
import { create_figure_scatter } from './widget-utils';

describe('scatter mega >', () => {
  beforeEach(async function () {
    this.manager = new DummyManager({ 'bqplot-gl': bqplot_gl });
  });
  ...
  it('size', async function () {
    ...
    expect(scatter.size.array).to.deep.equal(
      new Float32Array([default_size * 2])
    );
    expect(scatter.size.meshPerAttribute).to.equal(2);
    expect(scatter.size_previous.array).to.deep.equal(
      new Float32Array([default_size])
    );
    expect(scatter.size_previous.meshPerAttribute).to.equal(2);
    ...
    expect(scatter.size.array).to.deep.equal(
      new Float32Array([0, default_size * 2])
    );
  ...
  it('opacity', async function () {
    const x = {
      dtype: 'float32',
      value: new DataView(new Float32Array([0, 1, 2]).buffer),
    };
    const y = {
      dtype: 'float32',
    ...
    expect(scatter.opacity.array).to.deep.equal(
      new Float32Array([0.5, 1.0, 0.5])
    );
    ...
    expect(scatter.opacity.meshPerAttribute).to.equal(1);
    expect(scatter.opacity_previous.array).to.deep.equal(
      new Float32Array([0.5, 1.0, 0.5])
    );
  ...
  it('colors', async function () {
    const x = {
      dtype: 'float32',
      value: new DataView(new Float32Array([0, 1, 2, 1]).buffer),
    };
    const y = {
      dtype: 'float32',
      value: new DataView(new Float32Array([2, 3, 4, 3]).buffer),
    };
    const objects = await create_figure_scatter(this.manager, x, y);
    const scatter = objects.scatter;
    ...
    expect(scatter.material.defines['USE_COLORMAP']).to.be.false;
    ...
    expect(scatter.color.array).to.deep.equal(
      new Float32Array([
        red[0],
        red[1],
        red[2],
    ...
    expect(scatter.color.meshPerAttribute).to.equal(1);
    expect(scatter.material.defines['USE_COLORMAP']).to.be.true;
    expect(scatter.material.uniforms['domain_color'].value).to.deep.equal([
      new_color[3],
      new_color[1],
    ]);
  ...
  it('selection', async function () {
    ...
    expect(scatter.material.uniforms['has_selection'].value).to.be.false;
    ...
    expect(scatter.material.uniforms['has_selection'].value).to.be.true;
  ...
  it('markers number increase', async function () {
    ...
    scatter.model.set('x', new_x);
    expect(scatter.x.array).to.deep.equal(new_x);
    expect(scatter.x_previous.array).to.deep.equal(
      new Float32Array([0, 1, 10])
    );
