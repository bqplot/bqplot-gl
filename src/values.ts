...
export class Values {
  constructor(names, names_vec3, getter, sequence_index, names_vec4 = []) {
    ...
    for (const name of names) {
      const value = getter(name, sequence_index, this.defaults[name]);
    ...
    for (const name of names_vec3) {
      let value = getter(name, sequence_index, this.defaults[name]);
      ...
      if (isTypedArray(value) && value.length > 3) {
  ...
  trim(new_length) {
    this.array = lodash.mapValues(this.array, (array) => {
      return array.length === new_length ? array : array.slice(0, new_length);
    });
    this.array_vec3 = lodash.mapValues(this.array_vec3, (array_vec3) => {
      return array_vec3.length === new_length * 3
        ? array_vec3
        : array_vec3.slice(0, new_length * 3);
    });
    this.array_vec4 = lodash.mapValues(this.array_vec4, (array_vec4) => {
      return array_vec4.length === new_length * 4
        ? array_vec4
        : array_vec4.slice(0, new_length * 4);
    });
    this.length = new_length;
  }

  ensure_array(name_or_names) {
    const names = lodash.isArray(name_or_names)
      ? name_or_names
      : [name_or_names];
    for (const name of names) {
      ...
      if (typeof value_vec3 !== 'undefined') {
        const array = (this.array_vec3[name] = new Float32Array(
          this.length * 3
        ));
  ...
  pad(other) {
    this.array = lodash.mapValues(this.array, (array, name) => {
      const new_array = new array.constructor(other.length);
      if (typeof other.array[name] === 'undefined') {
    ...
    this.array_vec3 = lodash.mapValues(this.array_vec3, (array_vec3, name) => {
      const new_array = new array_vec3.constructor(other.length * 3);
      if (typeof other.array_vec3[name] === 'undefined') {
        // then other must be a scalar
        ...
        new_array.set(
          other.array_vec3[name].slice(this.length * 3),
          this.length * 3
        );
      }
      new_array.set(array_vec3);
    ...
    this.array_vec4 = lodash.mapValues(this.array_vec4, (array_vec4, name) => {
      ...
      if (typeof other.array_vec4[name] === 'undefined') {
        ...
        new_array.set(
          other.array_vec4[name].slice(this.length * 4),
          this.length * 4
        );
      }
  ...
  pop(name_or_names) {
    ...
    // _.each(names, (name) => {
    names.forEach((name, index) => {
      [this.scalar, this.scalar_vec3, this.array, this.array_vec3].forEach(
        (storage) => {
  ...
  add_attributes(geometry, postfix = '') {
    postfix = postfix;
    // set all attributes
    lodash.forOwn(this.array, (array, name) => {
    ...
    lodash.forOwn(this.array_vec3, (array, name) => {
      if (name.indexOf('selected') === -1) {
        // selected attributes should not be send to the shader
    ...
    lodash.forOwn(this.array_vec4, (array, name) => {
      if (name.indexOf('selected') === -1) {
        // selected attributes should not be send to the shader
    ...
    lodash.forOwn(this.scalar, (scalar, name) => {
      if (name.indexOf('selected') === -1) {
        // selected attributes should not be send to the shader
    ...
    lodash.forOwn(this.scalar_vec3, (scalar_vec3, name) => {
      if (name.indexOf('selected') === -1) {
        // selected attributes should not be send to the shader
        const attr = new THREE.InstancedBufferAttribute(
          scalar_vec3,
    ...
    lodash.forOwn(this.scalar_vec4, (scalar_vec4, name) => {
      if (name.indexOf('selected') === -1) {
        // selected attributes should not be send to the shader
        const attr = new THREE.InstancedBufferAttribute(
          scalar_vec4,
